#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
# [DEPRECATED - Phase 2 ASG 도입으로 사용 중지됨]
# dummy-web EC2 배포 스크립트
# 주의: 오토스케일링(ASG) 환경에서는 이 스크립트를 사용하지 않습니다!
# 대신 시작 템플릿(Launch Template)의 User Data가 직접 컨테이너를 기동합니다.
# 이 스크립트는 Phase 1 단일 서버 수동 배포 또는 로컬 EC2 테스트 용도로만 남겨둡니다.
# ─────────────────────────────────────────────────────────────
#
